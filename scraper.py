...
f = '{0}:\t{1}'	
print('https://tcgplayer.com')
print()

with urllib.request.urlopen('https://api.scryfall.com/cards/search?q=' + urllib.parse.quote(sys.argv[1])) as url:
    data = json.loads(url.read().decode())
for i in data['data']:
	print(i['name'])
	print(i['set_name'])
	print(f.format('USD', i['usd']))

print()
print('https://mtg.bigweb.co.jp')
print()
raw_html = simple_get('https://mtg.bigweb.co.jp/cards/filter?big_keyword=' + sys.argv[1])
...
for i in range(0, len(children)):
	if 'item' in children[i]['class']:
		...
		#Display Set Name
		print (children[i].findChildren()[0].findChildren()[1].findChildren()[0].text)